...
* **Main (GUI - Swing)**: Entry point and interface logic.
* **Playlist (Doubly Linked List)**: Core logic for list manipulation.
* **Node**: Individual units containing a `Song` object and `next`/`prev` links.
* **Song**: Data model for `title` and `artist`.
